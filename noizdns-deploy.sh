...
# Strip invisible/non-ASCII characters from input
sanitize_domain() {
...
generate_slipnet_configs() {
    ...
    local socks_pass="${INITIAL_PASS:-}"

    # If no fresh credentials but auth is enabled, show per-user links
    if [ -z "$socks_user" ] && grep -q "^socksmethod: username" /etc/danted.conf 2>/dev/null; then
        if [ -f "$USERS_FILE" ] && [ -s "$USERS_FILE" ]; then
            echo ""
            print_line
            echo -e "  ${BOLD}SlipNet Config Links${NC} (per-user, auth required)"
            print_line

            while IFS= read -r socks_user; do
                echo ""
                echo -e "  ${BOLD}User: ${socks_user}${NC}"
                local dnstt_data="16|dnstt|${short_name}|${NS_SUBDOMAIN}|${default_resolver}|0|5000|bbr|1080|127.0.0.1|0|${pubkey}|${socks_user}||||||0|127.0.0.1|0||udp|password|||0|443||||0||0|0|"
    ...
    local dnstt_data="16|dnstt|${short_name}|${NS_SUBDOMAIN}|${default_resolver}|0|5000|bbr|1080|127.0.0.1|0|${pubkey}|${socks_user}|${socks_pass}|||||0|127.0.0.1|0||udp|password|||0|443||||0||0|0|"
    local noizdns_data="16|sayedns|${short_name}|${NS_SUBDOMAIN}|${default_resolver}|0|5000|bbr|1080|127.0.0.1|0|${pubkey}|${socks_user}|${socks_pass}|||||0|127.0.0.1|0||udp|password|||0|443||||0||0|0|"
    echo ""
    echo -e "  ${CYAN}DNSTT:${NC}"
    echo -e "  ${WHITE}slipnet://$(echo -n "$dnstt_data" | base64 -w0)${NC}"
    echo ""
...
load_existing_config() {
    if [ -f "$CONFIG_FILE" ]; then
        . "$CONFIG_FILE"
        NS_SUBDOMAIN=$(sanitize_domain "$NS_SUBDOMAIN")
        return 0
    fi
    return 1
}
...
[Unit]
Description=NoizDNS SOCKS Server (dnstt -> Dante)
...
create_initial_user() {
    echo ""
    print_line
    echo -e "  ${BOLD}Create Initial User${NC}"
    print_line
    echo ""
    ...
    if [ "$AUTO_MODE" = true ]; then
        ...
        print_status "Auto-generated credentials:"
    else
        print_question "Create user now? [Y/n]: "
        read -r create_confirm
        if [[ "$create_confirm" =~ ^[Nn]$ ]]; then
            print_warning "Skipping — server will be open (no auth). Add a user later via User Management."
            return 0
        fi

        echo ""
        print_question "Username [user1]: "
        read -r username
        username=${username:-user1}

        if [[ ! "$username" =~ ^[a-z_][a-z0-9_-]*$ ]]; then
            print_error "Invalid username. Using 'user1'"
            username="user1"
        fi

        print_question "Password (empty = auto-generate): "
        read -rs password
        echo ""
        if [ -z "$password" ]; then
            password=$(generate_random_password)
            print_status "Auto-generated password"
        fi
    fi

    # Check if user already exists
    if id "$username" &>/dev/null || grep -qx "$username" "$USERS_FILE" 2>/dev/null; then
        print_warning "User '$username' already exists, skipping creation"
        return 0
    fi

    # Create Linux user — nologin shell for SOCKS-only
    ...
    # Save credentials temporarily for config link generation
...
add_user() {
    ...
    # Show config links for new user
    if [ -f "$PUBLIC_KEY_FILE" ] && [ -f "$CONFIG_FILE" ]; then
        load_existing_config
        local pubkey
        pubkey=$(cat "$PUBLIC_KEY_FILE" 2>/dev/null)
        ...
        local dnstt_data="16|dnstt|${short_name}|${NS_SUBDOMAIN}|${default_resolver}|0|5000|bbr|1080|127.0.0.1|0|${pubkey}|${username}|${password}|||||0|127.0.0.1|0||udp|password|||0|443||||0||0|0|"
        local noizdns_data="16|sayedns|${short_name}|${NS_SUBDOMAIN}|${default_resolver}|0|5000|bbr|1080|127.0.0.1|0|${pubkey}|${username}|${password}|||||0|127.0.0.1|0||udp|password|||0|443||||0||0|0|"

        echo ""
        print_line
        echo -e "  ${BOLD}Config links for ${username}${NC}"
        print_line
        echo ""
        echo -e "  ${CYAN}DNSTT:${NC}"
        echo -e "  ${WHITE}slipnet://$(echo -n "$dnstt_data" | base64 -w0)${NC}"
...
uninstall() {
    ...
    for svc in noizdns-server-ssh noizdns-dns noizdns-mux; do
    ...
    # Remove legacy SSH sshd_config Match Group block (if present from older installs)
    local sshd_config="/etc/ssh/sshd_config"
    if grep -q "# NoizDNS SSH tunneling" "$sshd_config" 2>/dev/null; then
        print_status "Removing legacy SSH tunnel configuration..."
    ...
    groupdel "noizdns-ssh" 2>/dev/null || true  # legacy SSH group cleanup
